...
ADR-0009 で informal が 5 種化（雑談追加）+ 進捗共有のログ単位許容に再改訂された。
...
| 状況共有 | 進捗共有 | 状態遷移単位（「サブリポ初期化完了 / マイグレーション通過」）でも、ログ単位（「ファイル X 編集中」「テスト残り 5 件」）でも可。粒度自由（ADR-0009 §2.3） |
| 仕様の不確実性解消 | 質疑応答 | 「Notion API の retry-after は秒？ms？」 |
| 設計選択肢の比較 | 議論 | 「ポーリング vs Webhook vs chokidar の 3 案比較。後で ADR のソースに使う想定」 |
| 業務外の軽い発話 | 雑談 | 「Webhook 再送基盤の小ネタ共有」「Drizzle 公式 release 雑感」等。タスクID・受領者すべて任意（ADR-0009 §2.2） |

**昇格規律**: 質疑応答が「業務継続に支障」レベルになったら問題報告へ。
議論で要所が決まったら歴史化メモ／ADR へ起案する（起案は Tech Lead）。雑談で要所判断が成立したら議論または ADR へ昇格。
**乱発禁止**: 雑談・進捗共有合計が掲示板全投稿の 70% 超になると ADR-0009 §2.5 ロールバック閾値に抵触。
