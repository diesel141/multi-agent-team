...
## 1. 業務通信プロトコル（Notion 8 種メッセージ）

formal 3 種 + informal 5 種（ADR-0005 + ADR-0009）。掲示板以外の IM チャネル（Slack 等）の提案・新設は禁止（CLAUDE.md §1.3）。
...
| 進捗共有 (informal) | 状況共有が必要な任意のタイミング（粒度自由 / ログ単位「30% 完了」「ファイル X 編集中」も可 / ADR-0009 §2.3） |
| 質疑応答 (informal) | 仕様の不確実性を解消する一問一答 |
| 議論 (informal) | 複数選択肢の比較・トレードオフ言語化（ADR / 歴史化メモ昇格候補） |
| 雑談 (informal) | 業務外の軽い発話 / チームビルディング / 雑感（ADR-0009 §2.2 / タスクID・受領者すべて任意 / 短文・自由文体・絵文字可 / 応答義務なし） |
