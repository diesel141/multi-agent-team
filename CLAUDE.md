...
ADR-0009 で **8 種化（雑談を追加）+ 進捗共有のログ単位許容** に再改訂された。

| 種別 | 用途 | 推奨 SLA / 投稿粒度 |
|------|------|---------------------|
| 着手返答 | 開始指令を受領した直後の「了解、着手します」 | 受領から 30 分以内 |
| 進捗共有 | 状況共有が必要な任意のタイミング | 粒度自由（ログ単位「30% 完了」「ファイル X 編集中」も可 / ADR-0009 §2.3） |
| 質疑応答 | 仕様の不確実性を解消する一問一答 | 業務継続に支障が出るレベルなら問題報告へ昇格 |
| 議論 | 複数選択肢の比較・トレードオフ言語化 | 後で ADR / 歴史化メモへ昇格する候補 |
| 雑談 | 業務外の軽い発話 / チームビルディング / 雑感 | 短文・自由文体・絵文字可 / 応答義務なし（ADR-0009 §2.2） |

#### 共通ルール

- すべての投稿で **「種別」プロパティは必須**（formal 3 種 + informal 5 種から 1 つ選択）
- formal の必須項目は不変。`docs/notion-board-schema.md` §2 のテンプレートに従う
- informal は文体・長さ自由。ただし掲示板以外の IM チャネル（Slack 等）の提案・新設は禁止
- 雑談は **タスクID・受領者すべて任意**（タイトルのみ必須）
- 詳細は `docs/adr/0005-communication-protocol-revision.md` / `docs/adr/0009-communication-protocol-re-revision.md` / `docs/notion-board-schema.md` を参照
...
- **phase2 復活条件**: ADR-0008 §3 却下 A 再考閾値（6 ロール以上 + 同時稼働 4 + 1 日 10 メッセージ超）が恒常化した時点で **将来の別 ADR**（ADR-0009 は通信プロトコル再改訂で消費済 / 番号は起案時の最新を採番）を起案して再導入
...
- 雑談・進捗共有を **乱発して掲示板の可読性を損なう**（ADR-0009 §2.5 ロールバック閾値: 雑談・進捗共有の合計が掲示板全投稿の 70% 超で再評価）
